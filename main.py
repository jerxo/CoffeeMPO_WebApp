...
st.set_page_config(page_title="Coffee Market Optimizer", layout='wide')
...
st.markdown("<h1 style='text-align: center'>Coffee Market Profit Optimizer</h1>", unsafe_allow_html=True)
st.write("---")
...
with st.expander("Coffee Farm General Information"):
    ...
    with col1:
        coffee_variety = st.radio("Select the variety of your Coffee:", ("Robusta", "Arabica", "Excelsa"))
    with col2:
        coffee_type = st.radio("Select the type of Coffee you will sell:", ("Fresh", "Green Coffee Beans", "Both"))
    with col3:
        ...
        if coffee_type == "Fresh":
            ...
            type_of_allocation = st.radio("Should all markets have an allocation?:", ("No", "Yes"))
        elif coffee_type == "Green Coffee Beans":
            amount_fc = 0
            amount_gcb = st.number_input(label="Enter the amount of Green Coffee Beans you will sell (in kilos)", min_value=0.00, step=5.00)
            st.subheader("Type of Allocation")
            type_of_allocation = st.radio("Should all markets have an allocation?:", ("No", "Yes"))
        elif coffee_type == "Both":
...
with maincol1:
    ...
    st.markdown("<p style='text-align: justify'>In this section please input your desired market allocation for your coffee products.</p>", unsafe_allow_html=True)
    if coffee_type == "Green Coffee Beans":
        tab1, tab2, tab3, tab4 = st.tabs(["Big Companies", "Traders", "Other Markets", "Cooperatives"])
        with tab1:
            st.subheader("Big companies (e.g. Nestle)")
            sellto_bigcompaniesgcb = st.radio("Will you sell to Big Companies (e.g. Nestle)?", ("Yes", "No"))
        ...
        with tab2:
            ...
            sellto_tradersgcb = st.radio("Will you sell to Traders?", ("Yes", "No"))
        ...
        with tab3:
            ...
            sellto_othermarketsgcb = st.radio("Will you sell to Other Markets (e.g. Supermarkets, Coffee Shops, etc.)?", ("Yes", "No"))
        ...
        with tab4:
            ...
            sellto_coopgcb = st.radio("Will you sell to a Cooperative?", ("Yes", "No"))
    ...
    if coffee_type == "Both":
        tab1, tab2 = st.tabs(["Fresh Coffee Cherries", "Green Coffee Beans"])
        with tab1:
            st.subheader("Traders")
            sellto_traders = st.radio("Will you sell fresh cherries to Traders?", ("Yes", "No"))
            if sellto_traders == "Yes":
                percent_traders = st.number_input(label="Enter the percentage of Fresh Coffee Cherries to be allocated to Traders", key="percent_traders", min_value=0.00, max_value=100.00, step=10.00)
                price_traders = st.number_input(label="Enter the price of Fresh Coffee Cherries when sold to Traders in pesos (php)", key="price_traders", min_value=0.00, step=10.00)
            elif sellto_traders == "No":
                percent_traders = 0
                price_traders = 0
            st.subheader("Other Markets (e.g. Supermarkets, Coffee Shops)")
            sellto_othermarkets = st.radio("Will you sell fresh cherries to Other Markets (e.g. Supermarkets, Coffee Shops) ?", ("Yes", "No"))
            if sellto_othermarkets == "Yes":
                percent_othermarkets = st.number_input(label="Enter the percentage of Fresh Coffee Cherries to be allocated to Other Markets", key="percent_othermarkets", min_value=0.00, max_value=100.00-percent_traders, step=10.00)
                price_othermarkets = st.number_input(label="Enter the price of Fresh Coffee Cherries when sold to Other Markets in pesos (php)", key="price_othermarkets", min_value=0.00, step=10.00)
            elif sellto_othermarkets == "No":
                percent_othermarkets = 0
                price_othermarkets = 0
        with tab2:
            st.subheader("Big companies (e.g. Nestle, etc.)")
            sellto_bigcompaniesgcb = st.radio("Will you sell to Big Companies (e.g. Nestle, etc.)?", ("Yes", "No"))
            if sellto_bigcompaniesgcb == "Yes":
                percent_bigcompaniesgcb = st.number_input(label="Enter the percentage of Green Coffee Beans to be allocated to Big Companies", key="percent_bigcompaniesgcb", min_value=0.00, max_value=100.00, step=10.00)
                price_bigcompaniesgcb = st.number_input(label="Enter the price of Green Coffee Beans when sold to Big Companies in pesos (php)", key="price_bigcompaniesgcb", min_value=0.00, step=10.00)
            elif sellto_bigcompaniesgcb == "No":
                percent_bigcompaniesgcb = 0
                price_bigcompaniesgcb = 0
            st.subheader("Traders")
            sellto_tradersgcb = st.radio("Will you sell to Traders?", ("Yes", "No"))
            if sellto_tradersgcb == "Yes":
                percent_tradersgcb = st.number_input(label="Enter the percentage of Green Coffee Beans to be allocated to Traders", key="percent_tradersgcb", min_value=0.00, max_value=100.00 - percent_bigcompaniesgcb, step=10.00)
                price_tradersgcb = st.number_input(label="Enter the price of Green Coffee Beans when sold to Traders in pesos (php)", key="price_tradersgcb", min_value=0.00, step=10.00)
            elif sellto_tradersgcb == "No":
                percent_tradersgcb = 0
                price_tradersgcb = 0
            ...
            sellto_othermarketsgcb = st.radio("Will you sell to Other Markets (e.g. Supermarkets, Coffee Shops, etc.)?", ("Yes", "No"))
            if sellto_othermarketsgcb == "Yes":
                percent_othermarketsgcb = st.number_input(label="Enter the percentage of Green Coffee Beans to be allocated to Other Markets", key="percent_othermarketsgcb", min_value=0.00, max_value=max_remaining_percentage, step=10.00)
                price_othermarketsgcb = st.number_input(label="Enter the price of Green Coffee Beans when sold to Other Markets in pesos (php)", key="price_othermarketsgcb", min_value=0.00, step=10.00)
            elif sellto_othermarketsgcb == "No":
                percent_othermarketsgcb = 0
                price_othermarketsgcb = 0
            ...
            sellto_coopgcb = st.radio("Will you sell to a Cooperative?", ("Yes", "No"))
            if sellto_coopgcb == "Yes":
                percent_coopgcb = st.number_input(label="Enter the percentage of Green Coffee Beans to be allocated to the Cooperative", key="percent_coopgcb", min_value=0.00, max_value=max_remaining_percentage, step=10.00)
                price_coopgcb = st.number_input(label="Enter the price of Green Coffee Beans when sold to the Cooperative in pesos (php)", key="price_coopgcb", min_value=0.00, step=10.00)
            elif sellto_coopgcb == "No":
                percent_coopgcb = 0
                price_coopgcb = 0
    ...
    if coffee_type == "Fresh":
        tab1, tab2 = st.tabs(["Traders", "Other Markets"])
        with tab1:
            st.subheader("Traders")
            sellto_traders = st.radio("Will you sell to Traders?", ("Yes", "No"))
            if sellto_traders == "Yes":
        ...
        with tab2:
            st.subheader("Other Markets (e.g. Supermarkets, Coffee Shops, etc.)")
            sellto_othermarkets = st.radio("Will you sell to Other Markets (e.g. Supermarkets, Coffee Shops, etc.)?", ("Yes", "No"))
            if sellto_othermarkets == "Yes":
...
with maincol2:
    ...
    st.markdown("<p style='text-align: justify'>In this section please input all the costs that you incurred.</p>", unsafe_allow_html=True)
    if coffee_type == "Fresh":
        tab1, tab2, tab3 = st.tabs(["Input Cost", "Labor Cost", "Other Cost"])
        with tab1:
    ...
    elif coffee_type == "Green Coffee Beans" or coffee_type == "Both":
        tab1, tab2, tab3, tab4 = st.tabs(["Input Cost", "Labor Cost", "Post Production Cost", "Other Cost"])
        with tab1:
            st.header("Input Costs:")
            st.subheader(f"Fertilizers")
            fertilizer1 = st.number_input(label="Enter the cost of Fertilizer 1 per kilogram: ", min_value=0.00, step=0.5)
